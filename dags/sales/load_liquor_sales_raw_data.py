...
file_prefix = Variable.get("LIQUOR_SALES_FILE_PREFIX", default_var='Liquor_Sales_')  # Configure for the specific file to pick
liquor_sales_raw_data_table_name = Variable.get("LIQUOR_SALES_RAW_DATA_TABLE_NAME", default_var='raw_liquor_sales_data')  # Configure the target table
liquor_sales_raw_data_table_sql = Variable.get("LIQUOR_SALES_TABLE_SQL") # This need to configure for SQL need to use to create table
...
# DAG for pipeline to download liquor sales raw data and ingest
dag = DAG(
...
# Task to check if the table exists and create it if not
...
# Task to download raw liquor sales data CSV files from S3 bucket to local data directory
